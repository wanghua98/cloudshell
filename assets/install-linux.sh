#!/usr/bin/env bash
#
# Install cloudshell's icon + desktop entry on Linux so the GNOME/Ubuntu dock and
...
# running window to its .desktop file via the window's app_id — cloudshell sets
# that to "cloudshell" (slint::set_xdg_app_id), and this script's StartupWMClass
# matches it.
#
# Usage:
#   ./install-linux.sh [/path/to/cloudshell-binary]
# You normally don't need an argument: when run from inside a release package
# (the `cloudshell` binary sits next to this script) it is picked up automatically.
# In the source tree it falls back to ./target/release/cloudshell.
...
if [ -n "${1:-}" ]; then
    BIN="$1"
elif [ -x "$SCRIPT_DIR/cloudshell" ]; then
    BIN="$SCRIPT_DIR/cloudshell"
else
    BIN="$SCRIPT_DIR/../target/release/cloudshell"
fi
...
if [ ! -x "$BIN" ]; then
    echo "error: cloudshell binary not found: $BIN" >&2
    echo "Run this script from the extracted release folder (it sits next to the" >&2
    echo "'cloudshell' binary), or pass the binary path as an argument." >&2
    exit 1
fi
...
StartupNotify=true
StartupWMClass=cloudshell
EOF
chmod 644 "$APP_DIR/cloudshell.desktop"
...
echo "Installed:"
echo "  icon    -> $ICON_DIR/cloudshell.png"
echo "  desktop -> $APP_DIR/cloudshell.desktop"
echo "  exec    -> $BIN"
echo
